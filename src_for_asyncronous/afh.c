...
uint8_t permute(uint8_t Z, uint16_t P) {
    ...
    if (P & (1 << 12)) { uint8_t temp = Z_out[0]; Z_out[0] = Z_out[3]; Z_out[3] = temp; }  // P12: {Z0, Z3}
    if (P & (1 << 13)) { uint8_t temp = Z_out[1]; Z_out[1] = Z_out[2]; Z_out[2] = temp; }  // P13: {Z1, Z2}

    if (P & (1 << 10)) { uint8_t temp = Z_out[2]; Z_out[2] = Z_out[4]; Z_out[4] = temp; }  // P10: {Z2, Z4}
    if (P & (1 << 11)) { uint8_t temp = Z_out[1]; Z_out[1] = Z_out[3]; Z_out[3] = temp; }  // P11: {Z1, Z3}

    if (P & (1 << 8))  { uint8_t temp = Z_out[1]; Z_out[1] = Z_out[4]; Z_out[4] = temp; }  // P8: {Z1, Z4}
    if (P & (1 << 9))  { uint8_t temp = Z_out[0]; Z_out[0] = Z_out[3]; Z_out[3] = temp; }  // P9: {Z0, Z3}

    if (P & (1 << 6))  { uint8_t temp = Z_out[0]; Z_out[0] = Z_out[2]; Z_out[2] = temp; }  // P6: {Z0, Z2}
    if (P & (1 << 7))  { uint8_t temp = Z_out[3]; Z_out[3] = Z_out[4]; Z_out[4] = temp; }  // P7: {Z3, Z4}

    if (P & (1 << 4))  { uint8_t temp = Z_out[0]; Z_out[0] = Z_out[4]; Z_out[4] = temp; }  // P4: {Z0, Z4}
    if (P & (1 << 5))  { uint8_t temp = Z_out[1]; Z_out[1] = Z_out[3]; Z_out[3] = temp; }  // P5: {Z1, Z3}

    if (P & (1 << 2))  { uint8_t temp = Z_out[1]; Z_out[1] = Z_out[2]; Z_out[2] = temp; }  // P2: {Z1, Z2}
    if (P & (1 << 3))  { uint8_t temp = Z_out[3]; Z_out[3] = Z_out[4]; Z_out[4] = temp; }  // P3: {Z3, Z4}

    if (P & (1 << 0))  { uint8_t temp = Z_out[0]; Z_out[0] = Z_out[1]; Z_out[1] = temp; }  // P0: {Z0, Z1}
    if (P & (1 << 1))  { uint8_t temp = Z_out[2]; Z_out[2] = Z_out[3]; Z_out[3] = temp; }  // P1: {Z2, Z3}
